...
class UWjuidate {
	...
	public function editAttribute($model,$field,$htmlOptions=array()) {
        ...
        $value = $model->{$field->varname};
        $model->{$field->varname} = $value=='0000-00-00'? date('Y-m-d') : $value;
        ob_start();
        $datapicker = Yii::app()->controller->widget('bootstrap.widgets.TbDatePicker',array(
            'id'=>!isset($htmlOptions['id']) ? $htmlOptions['id'] = get_class($model).'_'.$field->varname : $htmlOptions['id'],
            'model'=>$model,
            'value'=>  date('Y-m-d'),
