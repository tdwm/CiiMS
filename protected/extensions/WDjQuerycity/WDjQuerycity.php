...
class WDjQuerycity extends CInputWidget 
{
	...
	private $jsFiles			= array( '/src/jquerycity.js',);
	...
	*/
    ...
    public function run() 
    {
        $this->provinceOptions['id'] = $this->province;
        $this->cityOptions['id'] = $this->city;
        $this->areaOptions['id'] = $this->area;
        if($this->provinceOptions)
        {
            $this->provinceOptions =  array_merge($this->htmlOptions,$this->provinceOptions);
            $this->cityOptions = array_merge($this->htmlOptions,$this->cityOptions);
            $this->areaOptions = array_merge($this->htmlOptions,$this->areaOptions);
        }
        $provinceValue = array(
            ''=>'请选择省'
        );
        ...
        echo CHtml::activeDropDownList($this->model, $this->province, $provinceValue,  $this->provinceOptions); 
        echo CHtml::activeDropDownList($this->model, $this->city, $cityValue, $this->cityOptions); 
        echo CHtml::activeDropDownList($this->model, $this->area, $areaValue, $this->areaOptions); 
        parent::run();
    }
}
